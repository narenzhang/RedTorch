		...
		try {
			...
			if (beanFieldClazz==String.class) {
				...
				if(objectClzz==String.class) {
					...
					log.debug("Class-{}中成员变量{}的类型{}与当前值的类型{}不匹配,将尝试转换,可能丢失精度、溢出或出错",bean.getClass().getName(),varName,beanFieldClazz,objectClzz);
					try {
						String castingObject = object.toString();
						m.invoke(bean, castingObject);
					} catch (Exception e) {
						log.error("Class-{}中成员变量{}的类型{}与当前值的类型{}不匹配,强制转换出错",bean.getClass().getName(),varName,beanFieldClazz,objectClzz);
					}
				}
				...
				if(objectClzz==Date.class) {
					...
					log.error("Class-{}中成员变量{}的类型{}与当前值的类型{}不匹配,不可赋值",bean.getClass().getName(),varName,beanFieldClazz,objectClzz);
				}
			}
