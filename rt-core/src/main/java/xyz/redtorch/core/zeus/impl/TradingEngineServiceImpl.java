...
public class TradingEngineServiceImpl implements ZeusEngineService, InitializingBean {
	...
	public void sendOrder(OrderReq orderReq) {
		...
		getQueueTxEa().writeBytes(b -> b
				...
				.writeUtf8(orderReq.getGatewayID()).writeUtf8(orderReq.getSymbol()).writeUtf8(orderReq.getExchange())
				.writeUtf8(orderReq.getRtSymbol())

				.writeDouble(orderReq.getPrice()).writeInt(orderReq.getVolume()).writeUtf8(orderReq.getDirection())
				.writeUtf8(orderReq.getOffset()).writeUtf8(orderReq.getPriceType())

				.writeUtf8(orderReq.getOriginalOrderID())

				.writeUtf8(orderReq.getProductClass()).writeUtf8(orderReq.getCurrency()).writeUtf8(orderReq.getExpiry())
				.writeDouble(orderReq.getStrikePrice()).writeUtf8(orderReq.getOptionType())
				.writeUtf8(orderReq.getLastTradeDateOrContractMonth()).writeUtf8(orderReq.getMultiplier()));
	}
		...
		if (strategy == null) {
						...
						for (PositionDetail ydPositionDetail : ydPositionDetailList) {
							...
							if (contractPositionMap.containsKey(rtSymbol)) {
								...
								if (contractGatewayKeySet.contains(tmpContractGatewayKey)) {
									...
									log.error(strategy.getLogStr() + "从数据库中读取到合约" + rtSymbol + "接口ID" + gatewayID
											+ "组合与配置不匹配");
								}
							} else {
				...
				for (PositionDetail positionDetail : tdPositionDetailList) {
					...
					if (contractPositionMap.containsKey(rtSymbol)) {
						...
						if (contractGatewayKeySet.contains(tmpContractGatewayKey)) {
							...
							log.error(strategy.getLogStr() + "从数据库中读取到合约" + rtSymbol + "接口ID" + gatewayID + "组合与配置不匹配");
						}
					} else {
						log.error(strategy.getLogStr() + "从数据库中读取到配置中不存在的合约" + rtSymbol);
					}
			...
			for (StrategySetting.GatewaySetting GatewaySetting : strategy.getStrategySetting().getGateways()) {
				...
				for (String rtSymbol : GatewaySetting.getSubscribeRtSymbols()) {
					// 为策略注册Tick数据监听事件
					String subscribedTickKey = gatewayID + rtSymbol;
					...
					log.info(strategy.getLogStr() + "通过接口" + gatewayID + "订阅合约" + rtSymbol);
				}
			}
				...
				getQueueRxEt().readBytes(in -> {
					...
					if (ZeusMmapService.DATA_ORDER == dataType) {
						...
						if (originalOrderIDSet.contains(originalOrderID)) {
							...
							trade.setDateTime(new DateTime(in.readLong()));
							if (strategy != null) {
						...
						if (subscribedTickKeySet.contains(gatewayID + rtSymbol)) {
							Tick tick = new Tick();
