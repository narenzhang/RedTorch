...
@Service
public class ZeusMmapServiceImpl extends FastEventDynamicHandlerAbstract implements ZeusMmapService,FastEventDynamicHandler,InitializingBean{
	...
	public void afterPropertiesSet() throws Exception {
	    ...
	    queueTx = SingleChronicleQueueBuilder.binary(chronicleQueueBasePath+File.separator+"channel0").rollCycle(RollCycles.HOURLY).build();
	    queueTxEa = queueTx.acquireAppender();
	    
	    queueRx = SingleChronicleQueueBuilder.binary(chronicleQueueBasePath+File.separator+"channel1").rollCycle(RollCycles.HOURLY).build();
	...
	public void onOrder(Order order) {
		...
		if(StringUtils.isBlank(order.getOriginalOrderID())&&zeusTradingBaseService.getOriginalOrderID(order.getRtOrderID())!=null) {
	...
	public void onTrade(Trade trade) {
		if(StringUtils.isBlank(trade.getOriginalOrderID())&&zeusTradingBaseService.getOriginalOrderID(trade.getRtOrderID())!=null) {
		...
		getQueueTxEa().writeBytes(b->b
				...
				.writeLong(trade.getDateTime().getMillis()));
			...
			while(!Thread.currentThread().isInterrupted()) {
				getQueueRxEt().readBytes(in -> {
					...
					if(DATA_ORDERREQ == dataType) {
						...
						orderReq.setGatewayID(in.readUtf8());
						...
						if(StringUtils.isBlank(originalOrderID)||rtOrderID==null) {
							...
							Order order = coreEngineService.getOrder(zeusTradingBaseService.getRtOrderID(originalOrderID));
							if (order != null) {
